...
function useCountdown(dropAt) {
  ...
  useEffect(() => {
    if (!dropAt) return
    const calc = () => {
      const ms = new Date(dropAt) - new Date()
      setDiff(ms <= 0 ? 0 : ms)
    }
    calc()
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <h1 style={{
            ...
            lineHeight: 0.9, letterSpacing: '-0.02em', color: 'var(--white)', marginBottom: '24px',
          }}>
            SECURE<br />YOUR PAIR
          </h1>
...
function SneakerCard({ sneaker }) {
  ...
  useEffect(() => {
    const fetchStock = () =>
      productAPI.liveStock(sneaker.id)
        .then(d => setTotalStock(d.total_stock ?? null))
        .catch(() => {})
    fetchStock()

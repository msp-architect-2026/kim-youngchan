...
3. 화면 구성 및 API💻 화면 흐름 (UI Flow)로그인: JWT 기반 사용자 인증 및 보안 세션 유지상품 상세: Redis 캐싱을 통한 빠른 정보 조회 및 실시간 재고 확인주문/결제: 구매 클릭 시 Redis 기반 대기열 및 재고 검증 로직 진입⚙️ 주요 API 명세서비스엔드포인트설명AuthPOST /api/v1/auth/login사용자 로그인 및 토큰 발급ProductGET /api/v1/products/{id}상품 정보 및 현재 재고 조회OrderPOST /api/v1/orders핵심: Redis 재고 선차감 후 DB 저장4. 데이터베이스 모델링 (ERD)코드 스니펫erDiagram
    USERS {
        bigint id PK
        varchar email
        varchar password

...
# 🏗️ System Architecture Detail

본 프로젝트는 **On-premise 환경**에서 `kubeadm`을 활용해 구축된 쿠버네티스 클러스터를 기반으로 하며, **GitOps**와 **Observability**를 통합하여 기업급 인프라 운영 환경을 재현했습니다.

### 1. 시각적 구조 (Architecture Diagram)
...
graph TD
    ...
    subgraph "DevOps Infrastructure (On-premise Server)"
        GL[(GitLab)]
        GR[GitLab Runner / CI]
        CR[Container Registry]
    end

    subgraph "Kubernetes Cluster (kubeadm)"
        subgraph "Control Plane"
            ACD[Argo CD]
        end

        subgraph "Worker Nodes (Worker 1, 2)"
            OB[Online Boutique / MSA App]
            ING[Ingress-NGINX]
            MLB[MetalLB]
        end
    ...
    GR -- "Build & Push" --> CR
    GR -- "Update Tag" --> GL
    ACD -- "Sync Manifest" --> GL
    ACD -- "Deploy" --> OB
    MLB -- "L2 LoadBalancer" --> ING
    ING -- "Routing" --> OB
    PROM -- "Scrape Metrics" --> OB

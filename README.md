...
애플리케이션의 단순 기능을 넘어, 대규모 트래픽을 견디는 인프라 복원력과 데이터 정합성 확보를 목표로 합니다.
...
MetalLB + Nginx Ingress로 외부 트래픽 수용
...
고부하 Order Service에 Redis Atomic 연산(DECR) 적용
...
상품 목록/상세: Redis 캐싱을 통한 빠른 조회 및 실시간 재고 확인

주문/결제: 구매 클릭 시 Redis 기반 재고 검증 로직 진입
...
🧪 5. 검증 시나리오 (Test Scenarios)
 ...
 ArgoCD GitOps 동기화: 코드 변경 시 클러스터 자동 배포 확인

 HPA 확장 테스트: k6 부하 시 Pod 자동 증가 확인

 동시성 제어 테스트: 5,000 요청에서도 재고 정합성 유지

 장애 알림 테스트: 서비스 다운 시 Prometheus → Slack 알림 확인
